...
select 'ddlgen'
   ...
   +' -O$4/trigger/'+name+'.sql' -- output file name
   +' && sed -i ''/print/,$!d'' $4/trigger/'               
         +name+'.sql' -- remove all lines until 'print'         
   +' && sed -i ''/use '+db_name()+'/,/go/d'' $4/trigger/' 
         +name+'.sql' -- remove 'use' statement
   +' && sed -i ''/\-\- DDLGen Completed/,/\-\- at/d'' $4/trigger/'    
         +name+'.sql' -- remove comments at the end of file
   +' && sed -i ''/with dml_/,/ on/d'' $4/trigger/'       
         +name+'.sql' -- remove DML logging option
   +' && sed -i ''/with exp_row_size/,/ on/d'' $4/trigger/'       
         +name+'.sql' -- remove DML logging option
   +' && sed -i ''/sp_placeobject/,/go/d'' $4/trigger/'   
         +name+'.sql' -- remove 'sp_placeobject' statement
   +' && sed -i s/'+db_name()+'\.//g $4/trigger/'         
